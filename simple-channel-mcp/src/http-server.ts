...
export function startHttpServer(port: number) {
  ...
  wss.on('connection', (ws: WebSocket) => {
    ...
    ws.send(JSON.stringify({ type: 'status', state: sessionState } as WsServerMessage));
    ws.send(JSON.stringify({ type: 'activity', activity: state.activity } as WsServerMessage));
    if (Object.keys(state.sessionInfo).length > 0) {
      ws.send(JSON.stringify({ type: 'session_info', info: state.sessionInfo } as WsServerMessage));
    }
    for (const proc of state.backgroundProcesses.values()) {
      ws.send(JSON.stringify({ type: 'background_process', process: proc } as WsServerMessage));
    }
    for (const event of state.toolEvents) {
      ws.send(JSON.stringify({ type: 'tool_event', event } as WsServerMessage));
    }
    for (const message of state.messages) {
      ws.send(JSON.stringify({ type: 'message', message } as WsServerMessage));
    }
    ...
    ws.on('message', async (raw) => {
      ...
      try {
        const msg: WsClientMessage = JSON.parse(rawStr);
        if (msg.type === 'send') {
