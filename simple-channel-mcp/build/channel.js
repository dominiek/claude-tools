...
export function createChannelServer() {
    server = new Server({ name: 'simple-channel', version: '0.1.0' }, {
        capabilities: {
            experimental: {
                'claude/channel': {},
        ...
        switch (name) {
            ...
            case 'update_session_info': {
                state.setSessionInfo({
                    ...
                    tokens_limit: args.tokens_limit,
            ...
            case 'report_tool_use': {
                const tool_event_id = args.tool_event_id;
                const tool_name = args.tool_name;
                const input_summary = args.input_summary;
                const status = args.status;
...
export async function sendToChannel(content, meta) {
    const msg = {
        id: crypto.randomUUID(),
        timestamp: Date.now(),
        direction: 'inbound',
...
export async function sendPermissionVerdict(request_id, behavior) {
